...
本服务是一个纯 API 文件存储服务，基于 **FastAPI + SQLite + 内容寻址存储（CAS）** 架构，支持：

- 临时上传 / 永久上传
- 流式分片上传与断点续传
- 文件访问与下载（支持 HTTP Range）
- 文件列表查询（分页、多条件筛选）
- SHA-256 内容寻址存储 + 自动去重
- 12 位短字符串 fileId 标识文件
- 虚拟多级文件夹索引
- 加密文件夹与按会话解锁访问
- 受保护文件的临时下载直链
- 索引名重命名（同文件夹同扩展名唯一）
...
- 基于 `APP_CHANNEL`、`USER` 的审计日志记录（入库可查询）
- SQLite 数据库自动备份

服务端技术栈：

- **Web 框架**：FastAPI + uvicorn（异步）
- **数据库**：SQLite（WAL 模式）+ SQLAlchemy ORM + aiosqlite
- **文件存储**：内容寻址存储（CAS），`storage/files/{sha256前2位}/{sha256}.ext`

```text
storage/
  metadata.db           # SQLite 数据库（元数据）
  metadata.db-wal       # WAL 日志
  files/                # 内容寻址文件存储
    ab/
      abcdef...123.apk
  .chunks/              # 续传分片临时文件
  .cleanup_state        # 清理日期记录
  .db_backups/          # 数据库自动备份
```

**⚠️ 与 v2.x 不兼容**：v3.0 元数据结构、链接格式、存储路径全部重构，旧数据不会被新版本识别。
...
- **文件访问前缀**：`/p`（v3.0 变更，旧版为 `/images`）
- 数据库路径：`storage/metadata.db`（固定，不可通过环境变量修改）
...
| `GET` | `{FILE_ROUTE_PREFIX}/...` | 访问或下载真实文件（v3.0: `/p/{base64url}`） |
...
- 存储目录：`storage/files/{sha256前2位}/{sha256}.ext`（内容寻址存储）
- 返回路径格式：`/p/{base64url(systemName)}`（v3.0 新格式）
- 临时文件默认 24 小时后自动清理
...
```json
{
  ...
  "data": {
    ...
    "systemName": "a1b2c3d4e5f6...ff.png",
    "storage": "temporary",
    "size": 2048,
    "mimeType": "image/png",
    "sha256": "a1b2c3d4e5f6...ff",
    "extension": ".png",
    "path": "/p/YWJjZGVm...ZmYK",
    "url": "/p/YWJjZGVm...ZmYK",
    "folderId": null,
...
```json
{
  ...
  "data": {
    "fileId": "00000000000b",
    "indexedName": "安装包.apk",
    "systemName": "b2c3d4e5f6a1...ee.apk",
    "storage": "permanent",
    "size": 4096,
    "mimeType": "application/vnd.android.package-archive",
    "sha256": "b2c3d4e5f6a1...ee",
    "extension": ".apk",
    "path": "/p/YjJjM2Q0...ZWU",
    "url": "/p/YjJjM2Q0...ZWU",
    "folderId": null,
    "createdAt": "2026-07-11T01:05:00+00:00",
    "accessCount": 0
  }
}
```
